...
layout(location = 0) in vec2 aQuadVertPos;   // quad vertex positions
...
void main() {
    ...
    vec2 pos = iCharScreenPos + aQuadVertPos * uGlyphSize * 0.5;
    vec2 clipPos = (pos / uResolution) * 2.0 - 1.0;
    clipPos.y *= -1.0;
    gl_Position = vec4(clipPos, 0.0, 1.0);

    vec2 uvOffset = aQuadVertPos * uGlyphSize * 0.5;
